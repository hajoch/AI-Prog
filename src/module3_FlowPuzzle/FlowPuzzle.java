...
public class FlowPuzzle extends GACProblem{
	...
	private final int COVERAGEBONUS = 1;	//Coverage-bonus to reward longer journeys
...
//	public final int[][] DATA; 
	...
	public FlowPuzzle(int size, int flows, PuzzlePiece[][] puzzle, HashSet<? extends Variable> vars) {
		super(null, vars);
...
//		if(heuristics(node) != 0)
	//		return false;
		...
		return true;
/*		
		//Checks if all spaces in the grid are filled with flows
	...
	public float heuristics(Node node) {
		...
		// Combined Manhattan distance between the two variables of the same color.
		int total = 0;
		int coverage = 0;
		...
		for(Variable var : state.getVariables()) {
			...
			for(Variable var2 : state.getVariables()) {
				...
				if(fh1.color.equals(fh2.color)) {
					manhattenDistance += Math.abs(fh1.getCurrent().X - fh2.getCurrent().X);
					manhattenDistance += Math.abs(fh1.getCurrent().Y - fh2.getCurrent().Y);
					break;
				}
			}
	...
	/*	
		...
		for(FlowHead flowHead : (Set<FlowHead>)state.getVariables()) {
			if(!tested.contains(flowHead.color)) {
				...
				Point start = new Point(flowHead.getCurrent().X, flowHead.getCurrent().Y);
				Point goal = new Point(flowHead.getGoal().getCurrent().X, flowHead.getGoal().getCurrent().Y);
				...
				if(result.report() != Report.SUCCESS) {
	...
	*/	
		...
		while(!queue.isEmpty()) {
			...
			if(createdSingleton) {
				infinity--;
				//Move
				((FlowHead)revise.FOCAL).move((PuzzlePiece) revise.FOCAL.getDomain().get(0));
				try {
					Thread.sleep(0);
...
/*		for(Variable var : state.getVariables()){
	...
	private void handleMoveVariable(FlowState state, Variable focalO, boolean both) {
		
		if(both) {
			for(Constraint cNon : state.getConstraints()) {
				if(cNon.containsNonFocal(focalO.ID)) {
					Variable focal = state.getVariableByID((int)cNon.getVariableIDs().get("x"));
					Variable nonFocal = state.getVariableByID((int)cNon.getVariableIDs().get("y"));
		...
		for(Constraint c : state.getConstraints()) {
			if(c.containsFocal(focalO.ID)) {
				Variable focal = state.getVariableByID((int)c.getVariableIDs().get("x"));
				Variable nonFocal = state.getVariableByID((int)c.getVariableIDs().get("y"));
...
/*	private boolean moveSingletons(FlowState state) {
		boolean moved = false;
		for(Variable var : state.getVariables()) {
			if(var.isDomainSingleton()) {
				((FlowHead)var).move((PuzzlePiece) var.getDomain().get(0));
				moved = true;
				try {
					Thread.sleep(200);
					notifyListeners(state);
...
//				if(var.getDomainSize() == smallestDomain) {
					int manhattan 	 = Math.abs(var.getCurrent().X - var.getGoal().getCurrent().X);
					manhattan 		+= Math.abs(var.getCurrent().Y - var.getGoal().getCurrent().Y);
//					if(manhattan <= 3)
	//					smallDist++;
...
//			((ArrayList<Object>)succState.getVariableByID(assumedID).getDomain()).add(domainElement);
			...
			((FlowHead)newAssumed).move((PuzzlePiece) newAssumed.getDomain().get(0));
//			((FlowHead)succState.getVariableByID(assumedID)).move((PuzzlePiece) succState.getVariableByID(assumedID).getDomain().get(0));
...
//			domainFiltering(succNode);
		...
		/*
		for(Node succNodede : successors) {
			FlowState sta = (FlowState)succNodede.getState();
			for(FlowHead fh : (Set<FlowHead>)sta.getVariables()) {
//				fh.move((PuzzlePiece) fh.getDomain().get(0));
				queue.clear();
				handleMoveVariable(sta, fh, false);
				notifyListeners(sta);
				addVarsToRevise(sta);
...
/*		PriorityQueue<Variable> priQue = new PriorityQueue<Variable>();
		for(Variable v : state.getVariables()) {
			if(v.getDomainSize() > 1 && !((FlowHead)v).isConnected()) {
				priQue.add(v);
			}
		}
		...
		System.out.println("---------- NO NEW CHILDREN-----------");
		try {
			Thread.sleep(0);
		} catch (Exception e) {
			// TODO: handle exception
...
//		((FlowHead)revise.FOCAL).generateDomain();
		...
		for(Object focalDomObj : revise.FOCAL.getDomain()) {
			for(Object nonFocalPathObj : ((FlowHead)revise.NONFOCAL).getPath()) {
				if(((FlowHead)revise.FOCAL).getGoal().getCurrent() == (PuzzlePiece)nonFocalPathObj)
					continue;
				if(!Interpreter.interpret(revise.CONSTRAINT.RULE, new Object[]{focalDomObj, nonFocalPathObj})) {
...
/*		
		...
		for(Object focal : revise.FOCAL.getDomain()) {
			for(Variable v : revise.FOCAL.getNeighbors()) {
				for(Object nonFocal : ((FlowHead)v).getPath()) {
					if(((PuzzlePiece)focal).ID == ((PuzzlePiece)nonFocal).ID) {
...
/*		ArrayList<Object> toBeRemoved = new ArrayList<Object>();
		for(Object focal : revise.FOCAL.getDomain()) {
			for(Object nonFocal : ((FlowHead)revise.NONFOCAL).getPath()) {
